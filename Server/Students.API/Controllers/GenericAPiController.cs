...
// TODO: по хорошему, нажо заменить репозитории на сервисы и обрабатывать ошибку тут корреткно
/// <summary>
...
public abstract class GenericAPiController<TEntity> : ControllerBase where TEntity : class
{
    ...
    /// <summary>
    /// Частично обновить обьект
    /// </summary>
    /// <param name="id">Id объекта.</param>
    /// <param name="form">Объект.</param>
    /// <returns></returns>
    [HttpPatch("{id}")]
    public async Task<IActionResult> Patch(Guid id, [FromBody] TEntity form)
    {
        try
        {
            var patchResult = await this._rep.Patch(id, form);
            return patchResult is null ? this.NotFoundException() : this.Ok(form);
        }
        catch (Exception e)
        {
            this._logger.LogError(e, "Error while pacthing Entity");
            return this.Exception();
        }
    }
