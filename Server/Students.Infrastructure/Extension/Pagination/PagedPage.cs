...
public class PagedPage<T> where T : class
{
    ...
    public static async Task<PagedPage<T>> ToPagedPage<OrderKey>(IQueryable<T> source, int pageNumber, int pageSize,
        Func<T, OrderKey> orderKey)
    {
        var count = source.Count();
        // запрос без остлеживания изменений, но с отслеживаением удостоверений (уибрает дубликаты сущностей)
        var items = await source.AsNoTrackingWithIdentityResolution().Skip((pageNumber - 1) * pageSize).Take(pageSize)
            // разделяем один большой join на подзапросы
